...
export function AiCard({ tool, onEdit, onDelete, onClick }: AiCardProps) {
  ...
  return (
    <div 
      className="card group bg-glass rounded-xl overflow-hidden shadow-apple transition-all duration-300 hover:shadow-apple-hover border border-white border-opacity-40 cursor-pointer"
    ...
    >
      ...
      <div className="p-5">
        <div className="flex justify-between items-start">
          ...
          <span className={`px-2 py-1 bg-${CATEGORY_COLORS[tool.category as keyof typeof CATEGORY_COLORS]}-100 text-${CATEGORY_COLORS[tool.category as keyof typeof CATEGORY_COLORS]}-800 rounded-md text-xs font-medium`}>
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-2">
            {tool.tags.slice(0, 3).map((tag, index) => (
              <span key={index} className="px-2 py-0.5 bg-gray-100 text-apple-gray rounded-full text-xs font-medium">
                {tag}
              </span>
            ))}
            {tool.tags.length > 3 && (
              <span className="px-2 py-0.5 bg-gray-100 text-apple-gray rounded-full text-xs font-medium">
